...
class Attn(nn.Module):
    def __init__(self, hidden_size_input, hidden_size):
        ...
        self.attn1 = nn.Linear(self.hidden_size_input + self.hidden_size, self.hidden_size)
        self.attn2 = nn.Linear(self.hidden_size, 1)

    def forward(self, hidden, encoder_outputs, mask):
        """
        Compute the attention distribution from the encoder outputs
        at all timesteps, and the previous hidden state in the GRU.
        """
...
class Decoder(nn.Module):
    def __init__(self, hidden_size, embedding_size, output_size, bidirectional_input=False):
        super(Decoder, self).__init__()
        self.embedding_size = embedding_size
        self.hidden_size_input = hidden_size * 2 if bidirectional_input else hidden_size
        self.hidden_size = hidden_size
        ...
        self.gru = nn.GRU(self.hidden_size_input + self.embedding_size, self.hidden_size)
    ...
    def forward(self, _input, last_hidden, encoder_out, batch_size, use_cuda, mask):
        # Reshape to 1 x B x H (double check this
        # is the right way to do this...)
        embedded = self.embedding(_input).view(1, batch_size, self.hidden_size)
        ...
        gru_in = torch.cat((embedded, context), 2)
