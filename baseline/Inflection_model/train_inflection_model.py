...
def train(pairs, dev_pairs, lang, lang_label, setting, encoder, decoder, loss_function, optimizer, use_cuda, batch_size=100, epochs=20, lr=.01, clip=2):
    random.shuffle(pairs)
    train_batches = get_batches(pairs, batch_size, char2i, PAD_symbol, use_cuda)
    ...
    for i in range(epochs):
        ...
        for batch in train_batches:
            ...
            enc_out, enc_hidden = encoder(batch.input_variable.t())
            ...
            for t in range(1, batch.max_length_out):
                decoder_output, decoder_hidden= decoder(decoder_input,decoder_hidden, enc_out, batch.size,\
                ...
                loss = loss_function(decoder_output.squeeze(0), targets[t])
        ...
        print("LOSS: %4f" % (sum(all_losses)/ len(all_losses)))

        dev_acc = evaluate(encoder, decoder, char2i, dev_pairs, batch_size, PAD_symbol, use_cuda)
        print("ACC: %.2f %% \n" % dev_acc)
        
        # Overwrite saved model if dev acc is higher
        if dev_acc > last_dev_acc:
            print("saving ... ./models/%s/encoder-%s" % (setting, lang_label))
            torch.save(encoder, "./models/%s/encoder-%s" % (setting, lang_label))
            torch.save(decoder, "./models/%s/decoder-%s" % (setting, lang_label))
...
if __name__=='__main__':
    ...
    char_output = open('./models/%s/char2i-%s.pkl' % (setting, lang_label), 'wb')
    pickle.dump(char2i, char_output)
    
    loss_func = nn.NLLLoss(ignore_index=PAD_index, reduce=False)
    encoder = Encoder(input_size, EMBEDDING_SIZE, HIDDEN_SIZE, bidirectional=True)
    decoder = Decoder(HIDDEN_SIZE, EMBEDDING_SIZE, output_size, bidirectional_input=True)
    if use_cuda:
    ...
    pairs = [([EOS_symbol] + i + [EOS_symbol], [EOS_symbol] + o + [EOS_symbol])\
             for i, o in data.pairs]

    dev_pairs = [([EOS_symbol] + i + [EOS_symbol], [EOS_symbol] + o + [EOS_symbol])\
