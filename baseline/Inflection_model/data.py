...
class Data:
    ...
    def _read_data(self, fn):
        with codecs.open(fn, "r", encoding='utf-8') as file:
            ...
            #     print(lemma, wf, tags)
            #     pairs = [(list(lemma) + tags.split(";"), list(wf))]
...
class Batch:
    ...
    def make_output_variable(self, char2i, use_cuda):
        ...
        for i, o in enumerate(self.outputs):
            ids = [char2i.get(c, UNK_index) for c in o]
            ...
            # Pad the difference with symbol
            ids = ids + [char2i[self.symbol]] * (self.max_length_out - len(o))
            tensor[i] = torch.LongTensor(ids)

        output_var = Variable(tensor).cuda() if use_cuda else Variable(tensor)
...
def get_batches(pairs, batch_size, char2i, PAD_symbol, use_cuda, test_mode=False):
    ...
    batches = [Batch(sorted_pairs[i:i+batch_size], PAD_symbol) for i in range(0, len(sorted_pairs), batch_size)]
